...
const cardNumber = Object.keys(cardData).length;

class MiniSueca extends Game {
  constructor(params = {}) {
    super(params);

    let deck = MiniSueca.generateDeck();
  ...
  isEnded() { return this.error || this.winners !== null || this.tricksDone === cardNumber; }
  ...
  move(player, card) {
    if (!this.isValidMove(player, card)) {
      ...
      this.hands[player - 1] = _(this.hands[player - 1]).reject(MiniSueca.cardEquals(card));

      this.nextPlayer = MiniSueca.getPlayerAfter(player);
  ...
  onMoveTimeout() {
    this.winners = MiniSueca.getTeam(this.nextPlayer) === 1 ? [1, 3] : [2, 4];
    this.nextPlayer = null;
    return true;
  }

  _canPlay(player, card) {
    if (!_(this.hands[player - 1]).some(MiniSueca.cardEquals(card))) return false;
  ...
  _endTrick() {
    ...
    let winnerPlayer = this.currentTrick.findIndex(MiniSueca.cardEquals(winnerCard)) + 1;
    this.score[MiniSueca.getTeam(winnerPlayer) - 1] +=
