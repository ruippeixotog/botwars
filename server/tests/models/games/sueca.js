...
describe('Sueca', function() {
  ...
  it('should set the state as error when an invalid move is made', function () {
    game.move(game.getNextPlayer(), game.getState(nextPlayer(game.getNextPlayer())).hand[0]); // card not in player's hand
    assert.equal(game.isError(), true);

    startNewGame();
    game.move(nextPlayer(game.getNextPlayer()), validCard()); // not the correct player
    assert.equal(game.isError(), true);

    startNewGame();
    while(!invalidCard()) {
      game.move(game.getNextPlayer(), validCard());
      if(game.isEnded()) return; // this can happen, but requires an insanely rare card distribution
    }
    game.move(game.getNextPlayer(), invalidCard()); // card not from the trick's suit
    assert.equal(game.isError(), true);
  });
  ...
  it('should evaluate correctly valid and invalid moves', function () {
    ...
    assert.equal(game.isValidMove(game.getNextPlayer(),
        game.getState(nextPlayer(game.getNextPlayer())).hand[0]), false); // card not in player's hand

    assert.equal(game.isValidMove(nextPlayer(game.getNextPlayer()), validCard()), false); // not the correct player

    game.move(game.getNextPlayer(), validCard());
    assert.equal(game.isValidMove(game.getNextPlayer(), invalidCard()), false); // card not from the trick's suit
  });
